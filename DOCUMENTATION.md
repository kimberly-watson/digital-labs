...
| Emails going to spam | Sender (`kimberly.watson@sonatype.com`) is verified as an individual address, not a DKIM-authenticated domain. No email authentication for receiving mail servers to trust, so deliverability is unreliable. Needs proper domain verification (DKIM records for sonatype.com or a subdomain) before this goes to real customers — not fixed, just documented. |
...
- 2026-09-04: Added email notifications for Kimberly's own Gmail workflow:
  the portal Lambda now sends her a "[Digital Labs] New lab request: ..."
  email when a request is submitted, and the welcome email now CCs her
  and uses a matching "[Digital Labs]" subject prefix. Both exist so a
  Gmail filter can route them into a "Provisioning" label automatically.
  Added ses:SendEmail permission to the portal Lambda's IAM policy.
